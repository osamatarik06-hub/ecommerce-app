...
export default function AdminReviewsPage() {
  return (
    <div className="min-h-screen bg-[#FAF3E0] text-[#3B2F2F] flex flex-col justify-between font-sans">
      {/* Wrap Navbar in a Suspense boundary since it uses useSearchParams() */}
      <Suspense fallback={<div className="h-16 bg-[#3B2F2F]" />}>
        <Navbar />
      </Suspense>
      ...
      <main className="max-w-6xl mx-auto p-8 w-full space-y-6">
        ...
        <Suspense fallback={<div className="bg-white/95 border border-[#6F4E57]/30 rounded-3xl p-8 text-center text-sm text-[#6F4E57]">Loading reviews dashboard...</div>}>
          <ReviewsTable />
        </Suspense>
      </main>
    </div>
